...
_bool CVIBuffer_Cell::isPicking(const _matrix& WorldMatrix, _vector* pOut)
{	
	/* 마우스 레이와 레이포스를 로컬로 던졌다. */
	m_pGameInstance->Transform_MouseRay_ToLocalSpace(WorldMatrix);

	_vector pVecticesVec[3] = {
	XMLoadFloat4(&m_pVerticesPos[0]),
	XMLoadFloat4(&m_pVerticesPos[1]),
	XMLoadFloat4(&m_pVerticesPos[2])
	};
