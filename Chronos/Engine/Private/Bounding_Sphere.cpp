...
HRESULT CBounding_Sphere::Initialize(CBounding::BOUNDING_DESC* pBoundingDesc)
{
	...
	m_vInterval = _float3(pDesc->fRadius, pDesc->fRadius, pDesc->fRadius);
	return S_OK;
}
...
void CBounding_Sphere::Update(_fmatrix WorldMatrix)
{
	...
	m_isColl_AtLeastOnce = false;
}

HRESULT CBounding_Sphere::Render(PrimitiveBatch<VertexPositionColor>* pBatch)
{
#ifdef _DEBUG
	DX::Draw(pBatch, *m_pBoundingDesc, m_isColl_AtLeastOnce == true ? XMVectorSet(1.f, 0.f, 0.f, 1.f) : XMVectorSet(0.f, 1.f, 0.f, 1.f));
#endif
