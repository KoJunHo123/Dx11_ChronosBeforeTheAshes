...
CPlayer_Skill_Particle::CPlayer_Skill_Particle(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPartObject(pDevice, pContext)
{
}

CPlayer_Skill_Particle::CPlayer_Skill_Particle(const CPartObject& Prototype)
    : CPartObject(Prototype)
{
}

HRESULT CPlayer_Skill_Particle::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CPlayer_Skill_Particle::Initialize(void* pArg)
{
...
_uint CPlayer_Skill_Particle::Priority_Update(_float fTimeDelta)
{

    return OBJ_NOEVENT;
}

void CPlayer_Skill_Particle::Update(_float fTimeDelta)
{
...
HRESULT CPlayer_Skill_Particle::Render()
{
    // 월드 바인딩 x
    // 셰이더 만들고 테스트.
...
HRESULT CPlayer_Skill_Particle::Ready_Components()
{
    /* FOR.Com_Shader */
    ...
    /* FOR.Com_VIBuffer */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_VIBuffer_Particle_Skill"),
...
CPlayer_Skill_Particle* CPlayer_Skill_Particle::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CPlayer_Skill_Particle* pInstance = new CPlayer_Skill_Particle(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CPlayer_Skill_Particle"));
...
CGameObject* CPlayer_Skill_Particle::Clone(void* pArg)
{
    CPlayer_Skill_Particle* pInstance = new CPlayer_Skill_Particle(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CPlayer_Skill_Particle"));
        Safe_Release(pInstance);
    }
...
void CPlayer_Skill_Particle::Free()
{
    __super::Free();
