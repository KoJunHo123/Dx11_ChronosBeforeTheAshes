...
CParticle_Flash::CParticle_Flash(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CBlendObject(pDevice, pContext)
{
}

CParticle_Flash::CParticle_Flash(const CParticle_Flash& Prototype)
    : CBlendObject(Prototype)
{
}

HRESULT CParticle_Flash::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CParticle_Flash::Initialize(void* pArg)
{
...
_uint CParticle_Flash::Priority_Update(_float fTimeDelta)
{
    if (true == m_bDead)
        return OBJ_DEAD;
...
void CParticle_Flash::Update(_float fTimeDelta)
{
    ...
    if (false == m_bSmoke && m_fTexIndex < m_vDivide.x * m_vDivide.y - 15.f)
    {
        m_bSmoke = true;
    }
}

void CParticle_Flash::Late_Update(_float fTimeDelta)
{
...
HRESULT CParticle_Flash::Render()
{
...
HRESULT CParticle_Flash::Ready_Components()
{
    /* FOR.Com_Texture */
...
CParticle_Flash* CParticle_Flash::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CParticle_Flash* pInstance = new CParticle_Flash(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Create Failed : CParticle_Flash"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

CGameObject* CParticle_Flash::Clone(void* pArg)
{
    CParticle_Flash* pInstance = new CParticle_Flash(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Clone Failed : CParticle_Flash"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CParticle_Flash::Free()
{
    __super::Free();
